...
```
    ...
    sub       替换在字符串中匹配到的实例.
    subn      和 sub 相同, 但是也返回替换的统计数量.
    split     通过 pattern 划分字符串.
    findall   找到字符串中所有与 pattern 匹配的部分.
...
```py
    ...
    fullmatch(pattern, string, flags=0)
        """尝试在整个字符串上应用 pattern, 如果匹配成功, 返回一个 Match 对象, 否则返回 None"""
    ...
    split(pattern, string, maxsplit=0, flags=0)
        """通过 pattern 拆分字符串, 返回一个包含了结果子字符串的列表.
        如果在 pattern 中使用子表达式, 则匹配到的子表达式也会作为列表的一部分被返回.
        如果 maxsplit 非零, 则最多返回 maxsplit 个拆分, 剩余部分将作为列表最后一个元素返回."""
